...
//            $sql .= " LIMIT " . (int)$data['start'] . "," . (int)$data['limit'];
    ...
    {
        $query = $this->db->query("SELECT m.manufacturer_id as manufacturer_id FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_description md ON (m.manufacturer_id = md.manufacturer_id)  WHERE md.meta_title = '" . $this->db->escape($name) . "' OR m.name = '" . $this->db->escape($name) . "' ORDER BY CASE WHEN md.meta_title = '" . $this->db->escape($name) . "' THEN 0 ELSE 1 END");
