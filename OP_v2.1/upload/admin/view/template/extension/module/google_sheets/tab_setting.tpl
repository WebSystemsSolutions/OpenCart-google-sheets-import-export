                ...
                <?php if ('file' == $field['type']) { ?>
                    <div class="form-group">
                        <label class="col-sm-4 control-label"
                               for="<?= $field['type']; ?>-<?= $field_name ?>"><?= $field['label']; ?></label>
                        <div class="col-sm-6">
                            <input type="file" name="<?= $field_name ?>" value="<?= $field['value']; ?>"
                                   id="<?= $field['type']; ?>-<?= $field_name ?>" class="form-control" accept=".json"/>
                            <?php if ($field['file_uploaded']){ ?><p style="color: green">File uploading</p><?php } ?>
                        </div>
                        <div class="col-sm-2">
                            <button form="form-purchase" class="btn btn-primary">Load</button>
                        </div>
                    </div>
                <?php } ?>
    ...
    <div class="col-md-6">
        <div class="g-5">
            ...
            <a onclick="$(this).parent().find(':checkbox:not(:disabled)').prop('checked', true);">Выделить все</a> / <a
                    onclick="$(this).parent().find(':checkbox:not(:disabled)').prop('checked', false);">Снять
                выделение</a>
        </div>
